...
- Editable private catalog in `src/products.js` with all 21 products.
- Polished category-matched placeholders until real photos are added.
...
npm run build
```
The build writes deployable static files to `public/`, which matches the Vercel output directory configured in `vercel.json`.
